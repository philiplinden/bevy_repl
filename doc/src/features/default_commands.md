...
thing is that the REPL puts the terminal in "raw mode" and the cleanup ensures
that "raw mode" is disabled when the app exits. Otherwise, the terminal may
behave in unexpected ways even after the app has exited.
...
Shows all commands available to the REPL (generated by [`clap`](https://docs.rs/clap/latest/clap/))
