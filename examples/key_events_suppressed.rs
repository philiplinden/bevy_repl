...
fn instructions() {
    println!("The REPL is disabled, so key events are NOT forwarded to Bevy.");
    println!();
    println!(
        "Press Space, and see it added to the REPL input buffer. The Space
            keycode input does not trigger a ping from Bevy systems downstream
            of the REPL."
    );
    println!();
